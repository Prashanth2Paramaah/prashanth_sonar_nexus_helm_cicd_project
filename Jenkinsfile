pipeline{
 ...
 stage('Docker Image Build'){
      steps{
			...
			withCredentials([string(credentialsId: 'nexus_password', variable: 'nexus-credentials')]) {
		         sh '''
			 docker build -t 65.2.128.27:8085/springapp:${VERSION} .	 
	                 '''
			}
		}
 ...
 stage('Push Docker Image to Nexus Server'){
      steps{
			...
			withCredentials([string(credentialsId: 'nexus_password', variable: 'nexus-credentials')]) {
		         sh '''		 
			 docker login -u admin -p admin123 65.2.128.27:8085
			 
			 docker push 65.2.128.27:8085/springapp:${VERSION}
			 
			 docker rmi 65.2.128.27:8085/springapp:${VERSION}
	                 '''
			}
		}
